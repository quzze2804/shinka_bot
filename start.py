...
# Генерируем список доступных слотов с 08:00 до 17:00 с шагом 30 минут
...
async def cmd_book(message: Message):
    # Предлагаем выбрать дату (текущая дата и следующие 2 дня)
...
async def process_date(message: Message, state: FSMContext):
    ...
    # Генерируем доступные слоты на выбранную дату, отмечаем занятые
    slots = generate_slots(selected_date)
    keyboard = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    for slot in slots:
        if slot in booked_slots:
            # Помечаем как занятый (можно зачеркнуть или с ✖️)
            keyboard.add(KeyboardButton(text=f"❌ {slot.strftime('%H:%M')}"))
        else:
            keyboard.add(KeyboardButton(text=slot.strftime('%H:%M')))

    await message.answer(f"Оберіть зручний час для {selected_date}:", reply_markup=keyboard)
...
async def process_slot(message: Message, state: FSMContext):
    ...
    # Формируем datetime выбранного слота
...
async def process_confirmation(message: Message, state: FSMContext):
    ...
    admin_id = int((await bot.get("ADMIN_CHAT_ID", "7285220061")) or 7285220061)
...
async def handle_reschedule_select(message: Message, state: FSMContext):
    ...
    for dt in appts:
        if format_slot(dt) == text:
            ...
            await message.answer("Оберіть нову дату для перенесення:")
            await BookStates.waiting_for_date.set()
...
async def handle_reschedule_slot(message: Message, state: FSMContext):
    ...
    # Обновляем запись пользователя и забронированные слоты
    data = await state.get_data()
